...
import { createTaskRisk } from "./actions";
import { TaskRiskForm } from "./task-risk-form";

export default async function TaskRiskPage({
  params,
  searchParams,
}: {
  ...
  const assessments = await prisma.taskRiskAssessment.findMany({
    where: { projectId: id },
    ...
    return (
      <div className="min-h-full flex-1 bg-zinc-50 dark:bg-zinc-950">
        ...
        <main className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
          <Link href={`/projects/${id}/task-risk`} className="text-sm text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300">
            ← Task Risk Assessments
          </Link>
          <h1 className="mt-3 text-2xl font-semibold text-zinc-900 dark:text-zinc-50">New Task Risk Assessment</h1>
          <p className="mt-1 text-sm text-zinc-500">{safetyProject.name}</p>
          <div className="mt-8">
            <TaskRiskForm submitAction={createTaskRisk.bind(null, id)} />
          </div>
        </main>
      </div>
    );
    ...
    <div className="min-h-full flex-1 bg-zinc-50 dark:bg-zinc-950">
      ...
      <main className="mx-auto max-w-4xl px-4 py-10 sm:px-6">
        ...
        <div className="mt-3 flex items-start justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-2xl font-semibold text-zinc-900 dark:text-zinc-50">Task Risk Assessments</h1>
            <p className="mt-1 text-sm text-zinc-500">
              General-purpose risk assessment · hierarchy of controls enforced
            </p>
          </div>
          <Link
            href={`/projects/${id}/task-risk?new=1`}
        ...
        {assessments.length === 0 ? (
          <div className="mt-8 rounded-xl border border-zinc-200 bg-white p-8 text-center dark:border-zinc-800 dark:bg-zinc-900">
            <p className="text-sm text-zinc-500">No task risk assessments recorded yet.</p>
          </div>
        ) : (
          <div className="mt-8 space-y-4">
            {assessments.map((a) => {
              const controls = a.controls as { level: ControlLevel; description: string }[];
              const hazards = a.hazards as { hazard: string; riskRating: string }[];
              return (
                <div key={a.id} className="overflow-hidden rounded-xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
                  ...
                  <div className="px-5 py-4 space-y-3">
                    {hazards.length > 0 && (
                      <div>
                        <p className="text-xs font-medium text-zinc-500 mb-1">Hazards</p>
