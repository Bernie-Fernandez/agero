...
export async function POST(req: NextRequest) {
  ...
  const { first_name, last_name, email, phone, practice_type, message } = body as Record<string, string>;
  ...
  const { error } = await supabase.from("contacts").insert({
    ...
    job_title: practice_type?.trim() || null,
    legal_basis_for_data: message?.trim() || null,
    contact_type: "MBED_CONTACT",
    data_source: "API",
