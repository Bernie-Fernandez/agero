...
// status is NOT stored here — CAT does not export Awarded/Backlog.
// That classification is manual (Sprint X.2).
export type CatRow = {
  jobNo: string;
  projectName: string;
...
export type CatImportPreview = {
  ...
  parseErrors: string[];
  detectedAsAtDate?: string; // ISO YYYY-MM-DD extracted from banner row, if present
};
...
// Strips embedded newlines, collapses whitespace, lowercases.
// Applied to BOTH header cells and lookup keys before comparison.
...
const HEADER_MAP: Record<string, keyof CatRow> = {
  ...
  // WIP — real CAT exports use a combined "WIP\nOver Claim" cell header;
  // value goes into wip. Separate "Over Claim" column handled below.
  'wip':                        'wip',
  'wip over claim':             'wip',
  'overclaim':                  'overClaim',
...
function isSubtotalOrTotalsRow(cells: unknown[]): boolean {
  const first = String(cells[0] ?? '').trim();
  const second = String(cells[1] ?? '').trim();
  // Totals row: first cell empty, numeric data in later cells, no job number
  if (!first && !second) return false;
  return SUBTOTAL_PATTERNS.some((p) => p.test(first) || p.test(second));
}

// ── As-at date extraction from banner rows ────────────────────────────────────

const AS_AT_RE = /AS\s+AT\s+(\d{1,2})\/(\d{1,2})\/(\d{2,4})/i;

function extractAsAtDate(bannerRows: unknown[][]): string | undefined {
  for (const row of bannerRows) {
    for (const cell of row) {
      const s = String(cell ?? '');
      const m = AS_AT_RE.exec(s);
      if (m) {
...
export function parseCatExport(buffer: Buffer, filename: string): CatImportPreview {
  // xlsx auto-detects .xls vs .xlsx vs .csv from the buffer magic bytes
  ...
  // ── Locate header row: first row (within rows 0-9) containing 'job no' ──────
  let headerRowIdx = -1;
  let colMap: Map<keyof CatRow, number> | null = null;

  for (let i = 0; i < Math.min(10, raw.length); i++) {
    const candidate = raw[i] as unknown[];
    // Primary criterion: row must contain a cell that normalises to 'job no'
    const hasJobNo = candidate.some((c) => normaliseHeader(c) === 'job no');
    if (!hasJobNo) continue;

    // Build column map from this row
    const map = new Map<keyof CatRow, number>();
    for (let c = 0; c < candidate.length; c++) {
      const norm = normaliseHeader(candidate[c]);
      if (!norm) continue;
      const field = HEADER_MAP[norm];
      if (field && !map.has(field)) map.set(field, c);
    }

    // Secondary check: must have at least 3 known columns to look like a real CAT header
    if (map.size >= 3) {
      headerRowIdx = i;
      colMap = map;
      break;
  ...
  if (headerRowIdx === -1 || !colMap) {
    return {
      ...
      unmatchedHeaders: [],
      parseErrors: [
        "Could not locate header row containing 'Job No' in the first 10 rows of the file. " +
          'This file does not appear to be a CAT Project Financial Summary report.',
      ],
    };
  }

  // ── Extract as-at date from banner rows above the header ─────────────────────
  const detectedAsAtDate = extractAsAtDate(raw.slice(0, headerRowIdx));

  // ── Collect unmatched header names for the UI warning ────────────────────────
  const headerRow = raw[headerRowIdx] as unknown[];
  const unmatchedHeaders = headerRow
    .map((h) => normaliseHeader(h))
    .filter((n) => n && n.length > 2 && !HEADER_MAP[n]);

  // ── Helper to read a cell by field ───────────────────────────────────────────
  const colIdx = (field: keyof CatRow): number => colMap!.get(field) ?? -1;
  ...
  for (let i = headerRowIdx + 1; i < raw.length; i++) {
    ...
    if (isSubtotalOrTotalsRow(row)) {
      skippedRows++;
      continue;
    }

    const jobNo = getCell(row, 'jobNo');
    // Rows without a job number (e.g. the final totals row) are skipped
    if (!jobNo) {
      skippedRows++;
      continue;
    }
  ...
  return { rows, skippedRows, unmatchedHeaders, parseErrors: [], detectedAsAtDate };
}
