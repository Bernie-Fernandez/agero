...
export default function InspectionsClient({ initialData, projects }: { initialData: Inspection[]; projects: Project[] }) {
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-zinc-900">Safety Inspections</h1>
          <p className="text-sm text-zinc-500 mt-0.5">{filtered.length} of {data.length} inspections</p>
        </div>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16 text-zinc-400"><p className="text-sm">No inspections found.</p></div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-zinc-100 bg-zinc-50">
                ...
                <th className="text-left px-4 py-2.5 font-medium text-zinc-500 text-xs">Outcome</th>
                <th className="text-left px-4 py-2.5 font-medium text-zinc-500 text-xs">Inspector</th>
            ...
            <tbody>
              {filtered.map((i, idx) => (
                <tr key={i.id} className={`${idx < filtered.length - 1 ? 'border-b border-zinc-100' : ''} hover:bg-zinc-50`}>
                  ...
                  <td className="px-4 py-3 text-xs text-zinc-600">{fmt(i.inspectionDate)}</td>
                  <td className="px-4 py-3 text-xs">
                    <span className={`px-2 py-0.5 rounded-full font-medium ${STATUS_COLORS[i.status] ?? 'bg-zinc-100 text-zinc-600'}`}>{i.status}</span>
                  </td>
                  <td className="px-4 py-3 text-xs">
                    {i.outcome ? <span className={`px-2 py-0.5 rounded-full font-medium ${OUTCOME_COLORS[i.outcome] ?? 'bg-zinc-100 text-zinc-600'}`}>{i.outcome}</span> : <span className="text-zinc-400">—</span>}
                  </td>
                  <td className="px-4 py-3 text-xs text-zinc-600">{i.inspector ?? '—'}</td>
      ...
      <SlidePanel isOpen={addOpen} onClose={() => setAddOpen(false)} title="Add Inspection">
        <form action={async (fd) => {
          ...
          <div>
            <label className="block text-xs font-medium text-zinc-600 mb-1">Inspection Date <span className="text-red-500">*</span></label>
            <input name="inspectionDate" type="date" required defaultValue={new Date().toISOString().split('T')[0]}
          ...
          <div>
            <label className="block text-xs font-medium text-zinc-600 mb-1">Inspector Name</label>
            <input name="inspector" className="w-full border border-zinc-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand" />
          </div>
          <div>
            <label className="block text-xs font-medium text-zinc-600 mb-1">Outcome</label>
            <select name="outcome" defaultValue="" className="w-full border border-zinc-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand">
          ...
          <div>
            ...
            <textarea name="notes" rows={3} className="w-full border border-zinc-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand" />
          </div>
          <div className="flex gap-3 pt-2">
            <button type="submit" className="flex-1 px-4 py-2 bg-brand text-white text-sm font-medium rounded-md hover:opacity-90">Add Inspection</button>
