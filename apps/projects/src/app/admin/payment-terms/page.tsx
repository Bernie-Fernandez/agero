...
import { togglePaymentTermActive, deletePaymentTerm } from "./actions";
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-zinc-900">Payment Terms</h1>
          <p className="text-sm text-zinc-500 mt-1">
            Admin-managed options for company payment terms
          </p>
        </div>
        <Link
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50">
              ...
              <th className="text-left px-4 py-3 font-medium text-zinc-500 text-xs">Default</th>
          ...
          <tbody>
            {terms.map((t, idx) => (
              <tr
                key={t.id}
                className={`${idx < terms.length - 1 ? "border-b border-gray-100" : ""} ${!t.isActive ? "opacity-50" : ""}`}
              >
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2 justify-end">
                    <Link href={`/admin/payment-terms/${t.id}/edit`} className="text-xs text-blue-600 hover:underline">
                      Edit
                    </Link>
                    <form action={togglePaymentTermActive.bind(null, t.id, !t.isActive)}>
                      <button type="submit" className="text-xs text-zinc-500 hover:text-zinc-900">
                        {t.isActive ? "Deactivate" : "Activate"}
                      </button>
                    </form>
                    <form action={deletePaymentTerm.bind(null, t.id)}>
